            ...
            <React.Fragment key={i}>
              <div className={`flex flex-col items-center gap-1.5 ${i <= step ? 'opacity-100' : 'opacity-30'} transition-opacity duration-500`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg transition-all duration-500 ${
                  ...
                  i === step ? (darkMode ? 'bg-blue-500 shadow-lg shadow-blue-500/40' : 'bg-blue-600 shadow-lg shadow-blue-500/30') :
                  (darkMode ? 'bg-slate-800' : 'bg-slate-100')
                }`}>
                  {i < step ? '✓' : si}
                </div>
                <span className={`text-[10px] font-bold whitespace-nowrap ${
                  i === step ? (darkMode ? 'text-blue-400' : 'text-blue-600') : (darkMode ? 'text-slate-500' : 'text-slate-400')
                }`}>
                  {t(stepKeys[i] as any)}
                </span>
              </div>
