      ...
      <div className="max-w-4xl mx-auto flex flex-col gap-3 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[12px] font-bold text-slate-900 dark:text-slate-100">{t('selectionModeActive')}</span>
            <span className={`text-[11px] font-bold px-2 py-0.5 rounded-lg ${selectionMode === 'MATCH' ? 'bg-blue-100 dark:bg-blue-950/30 text-blue-500 dark:text-blue-400' : 'bg-rose-100 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400'}`}>
        ...
        <div className="flex gap-2">
          ...
          }} className={`flex-1 font-bold py-3 rounded-xl text-[12px] active:scale-95 transition-all ${selectedPlayerIds.length >= 2 ? (selectionMode === 'MATCH' ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/20' : 'bg-rose-500 text-white shadow-lg shadow-rose-500/20') : 'bg-slate-100 dark:bg-slate-800 text-slate-300 dark:text-slate-600'}`}>{t('apply')}</button>
          <button onClick={() => setSelectionMode(null)} className="flex-1 bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-bold py-3 rounded-xl text-[12px] active:scale-95 transition-all">{t('cancel')}</button>
        </div>
      </div>
    </div>
  );
