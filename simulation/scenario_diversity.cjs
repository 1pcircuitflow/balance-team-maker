// 농구 + 테니스 시뮬레이션 (다양성 포함)
// 다양성 = 이전 경기들과 비교하여 팀 구성이 얼마나 변경되었는지
...
// 다양성 계산 (이전 4경기 기록과 비교)
function calcDiversity(currentTeams, history) {
    if (history.length === 0) return 100; // 첫 경기는 100% 다양성
    ...
    currentPairs.forEach(pair => {
        if (historyPairs.has(pair)) overlap++;
    });

    // 다양성 = 100 - (겹치는 비율 * 100)
    const diversity = 100 - (overlap / currentPairs.size) * 100;
    return diversity;
}
...
function genBasketball() {
    ...
    for (const t of tiers) {
        for (let i = 0; i < t.count; i++) {
            ...
            p.push({ id: `B${id}`, skill: +skill.toFixed(1), pos: positions[id % 5], tier: t.tier });
            id++;
        }
    }
    return shuffle(p);
...
async function run() {
    ...
    //=========================================================================
    console.log('================================================================================');
    console.log('🏀 농구: 15명 → 3팀, 포지션쿼터, MATCH 3 + SPLIT 2');
    console.log('================================================================================\n');

    const bbStats = {};
    algos.forEach(a => bbStats[a.id] = { sdSum: 0, violSum: 0, divSum: 0, wins: 0, perfect: 0 });

    for (let run = 0; run < 500; run++) {
        const players = genBasketball();
        const constraints = genConstraints(players, 3, 2);

        // 이전 4경기 기록 생성 (다양성 측정용)
        const history = [];
        for (let h = 0; h < 4; h++) {
            const histPlayers = shuffle([...players]);
            const histTeams = Array.from({ length: 3 }, () => ({ players: [], totalSkill: 0 }));
            histPlayers.forEach((p, idx) => histTeams[idx % 3].players.push({ ...p }));
    ...
    console.log('순위 | 알고리즘  | 평균SD | 위반  | 다양성  | SD승리 | 완벽');
    console.log('-'.repeat(65));
    bbRank.forEach((r, i) => {
        const m = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : '  ';
        console.log(`${m}${i + 1}위 | ${r.id.padEnd(9)} | ${r.avgSD}  | ${r.avgViol}  | ${r.avgDiv}%  | ${r.wins}회  | ${r.perfect}`);
    });
    ...
    const tnStats = {};
    algos.forEach(a => tnStats[a.id] = { sdSum: 0, violSum: 0, divSum: 0, wins: 0, perfect: 0 });
    ...
    for (let run = 0; run < 500; run++) {
        ...
        for (let h = 0; h < 4; h++) {
            ...
            histPlayers.forEach((p, idx) => histTeams[idx % 5].players.push({ ...p }));
            history.push(histTeams);
        }
    ...
    console.log('🏆 최종 결론');
    console.log('================================================================================\n');
    console.log(`🏀 농구 1위: ${bbRank[0].id} (SD: ${bbRank[0].avgSD}, 다양성: ${bbRank[0].avgDiv}%)`);
    console.log(`🎾 테니스 1위: ${tnRank[0].id} (SD: ${tnRank[0].avgSD}, 다양성: ${tnRank[0].avgDiv}%)\n`);

    fs.writeFileSync('./scenario_diversity_results.json', JSON.stringify({ basketball: bbRank, tennis: tnRank }, null, 2));
    console.log('✅ 결과 저장: scenario_diversity_results.json\n');
}
